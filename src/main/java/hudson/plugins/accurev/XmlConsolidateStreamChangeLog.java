...
public class XmlConsolidateStreamChangeLog {
  ...
  private static final Map<Class<XMLOutputFactory>, XMLOutputFactory> OUTPUT_FACTORY_CACHE =
      new WeakHashMap<>(1);
  ...
  public static void createChangeLog(
      List<String> streamFiles, File changeLogFile, String updateFile) throws IOException {
    try (FileOutputStream changeLogStream = new FileOutputStream(changeLogFile)) {
      ...
      AccurevLauncher.logException(
          "Unable to create consolidated changelog "
              + XmlConsolidateStreamChangeLog.class.getSimpleName(),
          ex,
          logger,
          null);
    }
